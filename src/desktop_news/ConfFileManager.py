...
CONF_FILE_TEMPLATE = dict(
    ...
    prompt=dict(
        summary="below you will find a series of relevant news of today. could you summarize them and try to combine them into a single text with no more than 4 sentences?, i would also like you make all the necessary modifications to comply with dall-e's usage policies (avoid mention ethnicities, race, violent language, proper nouns and surnames and anything that could offend anyone):"
    ),
    response_path="/tmp/",
    image_path="~/images",
    image=dict(
